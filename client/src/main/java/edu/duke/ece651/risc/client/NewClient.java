...
public class NewClient {
     ...
     */
    public NewClient(Player player,
     ...
     */
    void sendOrders(OrderList ordersList, ObjectOutputStream out) throws IOException {
     ...
     */
    public void displayCommonMessage(ObjectInputStream in) throws IOException, ClassNotFoundException {
    ...
    public boolean validateAndAssignUnits(String units,
        ...
        try {
            ...
            if ((u > this.player.getAvailableUnits() - assignedUnits) || u <= 0 ||
                    (this.player.getAvailableUnits() - assignedUnits - u) < territoriesLeft) { // should be able to
                // accept 0
                outputWriter.println(
                        "Entered number of units unavailable or No units will be available for remaining territories.");
                return false;
            }
            // TODO - COMMENTED OUT: IMPLEMENT WITH NEW UNITS
            // T.addUnits(u);
            while (u > 0) {
                Unit bu = new BasicUnit();
                T.addUnit(0);
                u--;
            }
            return true;
    ...
    public Order createSingleUnitUpgradeOrder() {
        try {
            ...
            BasicOrder order = new UnitUpgradeOrder(this.player, source, this.unitTypesToBonus.get(currentUnitType),
                    this.unitTypesToBonus.get(targetUnitType));
            return order;
        } catch (IllegalArgumentException e) {
    ...
    public HashMap<Integer, Integer> getUnitsMap() throws IOException {
        ...
        while (true) {
            String type = getUnitType(
                    "Enter Unit type. Enter (O)ver to finish. Following are the available unit types:");
     ...
     */
    public void receivechecklosemap(ObjectInputStream in, ObjectOutputStream out)
     ...
     */
    public void winner(ObjectInputStream in) throws IOException, ClassNotFoundException {
    ...
    public void recieveTurnSummary(ObjectInputStream in) throws IOException, ClassNotFoundException {
